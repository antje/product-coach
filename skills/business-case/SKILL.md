---
name: business-case
description: Product financials coach. Translates product outcomes into P&L terms, reads a model you did not build with four checks, and writes a one-page business case with ranked assumptions, per-unit and volume returns, and a kill criterion that names a financial consequence. Includes a CFO stress test. Use when a bet needs funding, when reviewing someone else's numbers, or when asked to justify a decision financially.
---
