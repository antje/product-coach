...
- **Aim** is the North Star: the single metric that captures value delivered to the user and links to the business model. Direction-setting, slow-moving.
- **Move** is the leading indicators: metrics teams can shift this week that causally feed the Aim. This is where experiments run.
- **Prove** is the lagging outcomes: conversion, retention, revenue. They confirm; they arrive too late to steer by.
...
- **Cliff.** A sudden drop. Cause: a discrete recent event (release, pricing change, outage, tracking break). Response: find the event; check instrumentation before believing the product broke.
- **Slow Leak.** A gradual decline across cohorts. Cause: compounding erosion (quality, competition, mix shift). Response: cohort analysis to find when the leak started and which segment leaks.
