...
Put the paywall where an activated user has something to lose, and nowhere a non-activated user would ever feel it. Value tiers: free keeps foundational access sticky; paid gates the ongoing service (what the product watches, maintains, or guarantees); premium is expansion (seats, roles, scale). Users pay for what the product does for them continuously more readily than for permission to use features.

## Pricing experiments are different

- User-level A/B on price rarely randomizes cleanly (fairness, trust, leakage between users who compare receipts). Use **fresh signup cohorts** against prior-cohort baselines instead.
- Read **net revenue across billing cycles 2 and 3**, never the month-one spike; month one contains the announcement effect and the pull-forward.
- A price change is a **one-way door**: rolling back a price damages trust more than never changing it. Roll out by cohort, grandfather existing customers, and account for **revenue at risk** explicitly; the best designs put existing revenue at zero risk by construction.
- Success signal for a packaging line: conversions **cluster at the downgrade boundary**. Diffuse upgrades mean the line is in the wrong place; move the boundary, not the price.

## Expansion

Uniform revenue per customer (one flat plan) means net dollar retention is capped at customer retention. The structural fix is an expansion axis (seats, usage tiers, roles), queued behind proven activation, not bolted on before it.
