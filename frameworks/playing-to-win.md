...
**Vision** (aspirational, stable: who we serve, what problem, why now) → **Strategy** (how we deliver value, differentiate, make money; adaptable) → **Roadmap** (products, features, milestones) → **Execution** (build, launch, iterate; data feeds back up).

Each layer only makes sense in the context of the one above it. The most common failure is a roadmap with no strategy above it.

## The five-choice cascade

Every choice must reinforce the one above it. A capability that doesn't connect back to How to Win is noise.
...
4. **Can it be cascaded?** Can a team translate it directly into their own OKRs?
5. **Do the actions cohere?** Can you remove one initiative without affecting the rest? If yes, they don't cohere.
